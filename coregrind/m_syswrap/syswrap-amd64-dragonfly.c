...
/*--------------------------------------------------------------------*/
/*--- Platform-specific syscalls stuff.    syswrap-amd64-dragonfly.c ---*/
/*--------------------------------------------------------------------*/
...
#include "priv_syswrap-dragonfly.h"    /* for decls of dragonfly-ish wrappers */
...
/* ---------------------------------------------------------------------
   PRE/POST wrappers for amd64/Dragonfly-specific syscalls
   ------------------------------------------------------------------ */
...
PRE(sys_thr_new)
{
   ...
   /* Copy register state
      ...
      On Dragonfly, thr_new arranges a direct call.  We don't actually need any
      of this gunk.
...
PRE(sys_fake_sigreturn)
{
   ...
   SET_STATUS_from_SysRes( VG_(mk_SysRes_amd64_dragonfly)( tst->arch.vex.guest_RAX,
...
PRE(sys_sysarch)
{
   ...
   case VKI_AMD64_SET_FSBASE:
      ...
      /* On Dragonfly, the syscall loads the %gs selector for us, so do it now. */
      tst = VG_(get_ThreadState)(tid);
      p = (void**)ARG2;
...
#endif /* defined(VGP_amd64_dragonfly) */
