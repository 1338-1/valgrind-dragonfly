...
static HChar** setup_client_env ( HChar** origenv, const HChar* toolname)
{
   ...
   HChar* preload_string     = VG_(malloc)("initimg-dragonfly.sce.1",
                                           preload_string_len);
   vg_assert(origenv);
   vg_assert(toolname);
   ...
   /* Determine if there's a vgpreload_<tool>_<platform>.so file, and setup
      preload_string. */
   preload_tool_path = VG_(malloc)("initimg-dragonfly.sce.2", preload_tool_path_len);
   ...
   ret = VG_(malloc) ("initimg-dragonfly.sce.3",
   ...
   *(cpp + 1) = NULL;
   ...
   for (cpp = ret; cpp && *cpp; cpp++) {
      if (VG_(memcmp)(*cpp, ld_preload, ld_preload_len) == 0) {
         ...
         HChar *cp = VG_(malloc)("initimg-dragonfly.sce.4", len);
         vg_assert(cp);
   ...
   if (!ld_preload_done) {
      ...
      HChar *cp = VG_(malloc) ("initimg-dragonfly.sce.5", len);
      vg_assert(cp);
   ...
   if (VG_(is32on64)()) {
      for (cpp = ret; cpp && *cpp; cpp++) {
        if (VG_(memcmp)(*cpp, ld_32_preload, ld_32_preload_len) == 0) {
           Int len = VG_(strlen)(*cpp) + preload_string_len;
           HChar *cp = VG_(malloc)("initimg-dragonfly.sce.4a", len);
           vg_assert(cp);
      ...
      if (!ld_32_preload_done) {
        ...
        HChar *cp = VG_(malloc) ("initimg-dragonfly.sce.5a", len);
        vg_assert(cp);
