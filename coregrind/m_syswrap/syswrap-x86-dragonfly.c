...
#include "priv_syswrap-dragonfly.h"      /* for decls of linux-ish wrappers */
...
extern
Int do_syscall_clone_x86_dragonfly ( Word (*fn)(void *), 
                                 void* stack, 
                                 Int   flags, 
                                 void* arg,
                                 Int*  child_tid, 
...
asm(
".text\n"
"do_syscall_clone_x86_dragonfly:\n"
"        push    %ebx\n"
"        push    %edi\n"
"        push    %esi\n"
                         ...
                         UInt flags)
{
   ...
   */
   ...
   eax = do_syscall_clone_x86_dragonfly(
            ML_(start_thread_NORETURN), stack, flags /*, &VG_(threads)[ctid], NULL*/ );
   res = VG_(mk_SysRes_x86_dragonfly)( eax ); /* XXX edx returns too! */
