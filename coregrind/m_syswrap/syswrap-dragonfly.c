...
/*--------------------------------------------------------------------*/
/*--- Dragonfly-specific syscalls, etc.            syswrap-dragonfly.c ---*/
/*--------------------------------------------------------------------*/
...
#include "priv_syswrap-dragonfly.h"
...
static VgSchedReturnCode thread_wrapper(Word /*ThreadId*/ tidW)
{
   ...
   VG_(debugLog)(1, "syswrap-dragonfly", 
...
/* On Dragonfly, if any thread calls exit(2), then they are all shut down, pretty
...
// Pipe on dragonfly doesn't have args, and uses dual returns!
PRE(sys_pipe)
{
   PRINT("sys_pipe ()");
}
